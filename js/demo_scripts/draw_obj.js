...
Demo.prototype = {
    ...
    *******************************/
    ...
    processSdpNode : function(curIdx, sem, drawStruct) {
        ...
        drawStruct.sdp.push(curSdpNode);
    },
	...
	initDrawStruct : function (sentObj) {
		...
		for (i = 0; i < sentObj.length; i++) {
            ...
            this.processSdpNode(i, sentObj[i]["sem"], this.drawStruct);
		}
    ...
    *******************************/
    ...
    drawSdpNode : function(sdpNode, drawStruct, cxt){
        if(sdpNode.length == 0){
            return 0;
        }
        var oneSdpArc,
            ...
            ;
        // draw every line
        for(var i = 0; i < sdpNode.length; ++i){
            oneSdpArc = sdpNode[i];
            fromCenter = ( 
            ...
            controlPointY = lineY - controlPointHeight;
            actualY = lineY - controlPointHeight * 3 / 4;
            cxt.strokeStyle = drawStruct.SDP_DISCONF.lineColor;
            cxt.beginPath(); // !!!! must clear previous path ! or previous path will be repaint again and again , then the line become ugly with jaggies
            cxt.moveTo(fromX, lineY);
            cxt.bezierCurveTo(fromX, controlPointY, toCenter, controlPointY, toCenter, lineY);
            cxt.stroke();
            cxt.closePath();
            //draw relate text
            cxt.fillStyle = drawStruct.SDP_DISCONF.textColor ;
            cxt.textAlign = "center";
            this.setEnTextFont(cxt);
        ...
        arrowLeftOffset = sdpNode.length > 1 ? (arrowWidth - cxt.lineWidth) / 2 
                                             : sdpNode[0].to > sdpNode[0].from ? (arrowWidth - cxt.lineWidth) / 2 + 1 
        ...
        cxt.lineTo(toCenter + (arrowWidth - arrowLeftOffset), paintY - arrowHeight);
        cxt.closePath();
        cxt.fillStyle = drawStruct.SDP_DISCONF.lineColor ;
        cxt.fill();
        return actualY;
    },
    drawSDP : function(drawStruct){
		...
		for (i = 0; i < drawStruct.sdp.length; i++) {
			var yPos = this.drawSdpNode(drawStruct.sdp[i], drawStruct, cxt);
			if (minY > yPos)
				minY = yPos;
		}
		minY -= 10; //  the up margin
